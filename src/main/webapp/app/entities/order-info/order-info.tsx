  ...
  return (
    <div>
      <h2 id="order-info-heading" data-cy="OrderInfoHeading">
        <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.home.title">Order Infos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.home.createLabel">Create new Order Info</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderInfoList && orderInfoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th>
                  <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.product">Product</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.order">Order</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {orderInfoList.map((orderInfo, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{orderInfo.pricePerUnit}</td>
                  <td>{orderInfo.createAt ? <TextFormat type="date" value={orderInfo.createAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{orderInfo.modifyAt ? <TextFormat type="date" value={orderInfo.modifyAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    {orderInfo.promotionInfo ? (
                      <Link to={`promotion-info/${orderInfo.promotionInfo.id}`}>{orderInfo.promotionInfo.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>{orderInfo.product ? <Link to={`product/${orderInfo.product.id}`}>{orderInfo.product.id}</Link> : ''}</td>
                  <td>{orderInfo.order ? <Link to={`order/${orderInfo.order.id}`}>{orderInfo.order.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationApp.orderInfo.home.notFound">No Order Infos found</Translate>
            </div>
          )
        )}
      </div>
