  ...
  return (
    <div>
      <h2 id="promotion-heading" data-cy="PromotionHeading">
        <Translate contentKey="jhipsterSampleApplicationApp.promotion.home.title">Promotions</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplicationApp.promotion.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplicationApp.promotion.home.createLabel">Create new Promotion</Translate>
          </Link>
        </div>
      </h2>
        ...
        {promotionList && promotionList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.promotion.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('promotionDescription')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.promotion.promotionDescription">Promotion Description</Translate>{' '}
                  <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('promorionFormular')}>
            ...
            <tbody>
              {promotionList.map((promotion, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{promotion.promorionFormular}</td>
                  <td>{promotion.expireAt ? <TextFormat type="date" value={promotion.expireAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{promotion.createAt ? <TextFormat type="date" value={promotion.createAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{promotion.modifyAt ? <TextFormat type="date" value={promotion.modifyAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationApp.promotion.home.notFound">No Promotions found</Translate>
            </div>
          )
        )}
      </div>
