...
public interface ProductRepository extends R2dbcRepository<Product, Long>, ProductRepositoryInternal {
    ...
    @Query("SELECT * FROM product entity WHERE entity.inventory_id = :id")
    Flux<Product> findByInventory(Long id);

    @Query("SELECT * FROM product entity WHERE entity.inventory_id IS NULL")
    Flux<Product> findAllWhereInventoryIsNull();
